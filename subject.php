...
if($is_update){$r=db_fetch("SELECT * FROM subject WHERE sid=?",[$_GET['update']]);if($r){$sid=$r['sid'];$title=$r['title'];$description=$r['description'];}}
if(isset($_POST['submit'])){
  $sid=$_POST['sid'];$title=$_POST['title'];$description=$_POST['description'];
  try{
    if($is_update){db_query("UPDATE subject SET title=?,description=? WHERE sid=?",[$title,$description,$sid]);$message='<div class="alert alert-success alert-auto"><i class="fa fa-check-circle"></i> Subject updated!</div>';}
    else{db_query("INSERT INTO subject(sid,title,description) VALUES(?,?,?)",[$sid,$title,$description]);$message='<div class="alert alert-success alert-auto"><i class="fa fa-check-circle"></i> Subject added!</div>';}
...
<title>Subjects - ICST Academic Management</title><link rel="icon" href="images/user.png">
