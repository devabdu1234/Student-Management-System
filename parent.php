...
if($is_update){$r=db_fetch("SELECT * FROM parent WHERE pid=?",[$_GET['update']]);if($r){$pid=$r['pid'];$fname=$r['fname'];$lname=$r['lname'];$email=$r['email'];$contact=$r['contact'];$nic=$r['nic'];$gender=$r['gender'];$address=$r['address'];$job=$r['job'];}}
if(isset($_POST['submit'])){
  ...
  try{
    if($is_update){db_query("UPDATE parent SET fname=?,lname=?,address=?,gender=?,job=?,contact=?,email=?,nic=? WHERE pid=?",[$fname,$lname,$address,$gender,$job,$contact,$email,$nic,$pid]);$message='<div class="alert alert-success alert-auto"><i class="fa fa-check-circle"></i> Parent updated!</div>';}
    else{db_query("INSERT INTO parent(fname,lname,address,gender,job,contact,nic,email) VALUES(?,?,?,?,?,?,?,?)",[$fname,$lname,$address,$gender,$job,$contact,$nic,$email]);$message='<div class="alert alert-success alert-auto"><i class="fa fa-check-circle"></i> Parent added!</div>';}
