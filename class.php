<?php
/* class.php — Add, update, delete, and list classroom/lecture hall records */
...
if($is_update){
  // Fetch existing classroom for editing
  $r=db_fetch("SELECT * FROM classroom WHERE hno=?",[$_GET['update']]);if($r){$hno=$r['hno'];$title=$r['title'];$location=$r['location'];$capacity=$r['capacity'];}}
if(isset($_POST['submit'])){
  $hno=$_POST['hno'];$title=$_POST['title'];$location=$_POST['location'];$capacity=$_POST['capacity'];
  try{
    if($is_update){
      // Update existing classroom
      db_query("UPDATE classroom SET title=?,location=?,capacity=? WHERE hno=?",[$title,$location,$capacity,$hno]);$message='<div class="alert alert-success alert-auto"><i class="fa fa-check-circle"></i> Classroom updated!</div>';}
    else{
      // Insert new classroom
      db_query("INSERT INTO classroom(hno,title,location,capacity) VALUES(?,?,?,?)",[$hno,$title,$location,$capacity]);$message='<div class="alert alert-success alert-auto"><i class="fa fa-check-circle"></i> Classroom added!</div>';}
...
// Retrieve all classrooms for table listing
